      ...
      {tab === "overview" && (
        <div className="admin-grid">
          ...
          <div className="panel">
            ...
            <div className="seat-admin-list">{room.slots.map((seat) => { const link = links?.seatUrls.find((item) => item.slotId === seat.id)?.url; return <div key={seat.id}><span className="avatar small">{seat.ordinal}</span><div><strong>{seat.lastDisplayName || `Seat ${seat.ordinal}`}</strong><small>{link ? "Link available this browser session" : "Secret link hidden"}</small></div>{link && <CopyButton value={link} />}<button className="bare-button" disabled={Boolean(current)} onClick={() => void regenerate(seat.id)} title={current ? "Seat links are locked during an active occurrence" : "Regenerate seat link"}><RotateCw size={16} /></button></div>; })}</div>
      ...
      {tab === "settings" && (
        <div className="settings-stack">
          <form className="panel settings-form" onSubmit={(event) => void saveSettings(event)}>
            ...
            <div className="field-grid"><label>Role<select disabled={Boolean(current)} value={draft.role} onChange={(event) => setDraft({ ...draft, role: event.target.value as RoleType })}><option value="SCRUM_MASTER">Scrum Master</option><option value="FUN_FRIDAY">Fun Friday Organizer</option><option value="BRAINSTORM">Brainstorm Facilitator</option><option value="CUSTOM">Custom</option></select></label>{draft.role === "FUN_FRIDAY" ? <label>Game<select disabled={Boolean(current)} value={draft.game} onChange={(event) => setDraft({ ...draft, game: event.target.value as GameType })}><option value="AUTO">Auto</option><option value="RAPID_FIRE_TRIVIA">Rapid-fire trivia</option><option value="WOULD_YOU_RATHER">Would You Rather</option><option value="CATEGORIES">Categories</option></select></label> : <div />}</div>
            <label>How should the agent run it?<textarea maxLength={8000} rows={6} disabled={Boolean(current)} value={draft.instructions} onChange={(event) => setDraft({ ...draft, instructions: event.target.value })} /></label>
