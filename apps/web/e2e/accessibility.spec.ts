...
import { createRoom, loginPage } from "./helpers";
...
test("secure login is WCAG AA clean, keyboard reachable, and responsive", async ({ page }) => {
...
test("admin dashboard and participant entry are WCAG AA clean", async ({ page, request }, testInfo) => {
...
test("authenticated workspace is WCAG AA clean and responsive", async ({ page }) => {
  await loginPage(page);
  const fitsViewport = await page.evaluate(
    () => document.documentElement.scrollWidth <= window.innerWidth,
  );
